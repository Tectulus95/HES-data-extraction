import os
import sys
sys.path.append('hesDataExtraction')
...
thorRel = thor[(thor["TGTCOUNTRY"] == "SOUTH VIETNAM") & (thor["NUMWEAPONSDELIVERED"] > 0) & (thor["WEAPONTYPEWEIGHT"] > 0)].dropna(subset="TGTORIGCOORDS")

thorRel = gpd.GeoDataFrame(thorRel)
...
savepath = os.path.join(dirname, 'thor_relevant.json')
thorRel.to_file(savepath, driver='GeoJSON')
